...
import { exportARAgingToExcel } from '../../utils/excelExport';
import {
  AgingReportRow,
  getDocumentLabel,
  getPartyLabel,
...
const ARAgingReport: React.FC = () => {
  ...
  const loadAgingReport = async () => {
    ...
    try {
      const data = await accountingService.getARAgingReport(organizationId, asOfDate);
  ...
  return (
    <Box p={3}>
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
        <Typography variant="h4">AR Aging Report</Typography>
        <Button
          variant="contained"
          startIcon={<DownloadIcon />}
          onClick={() => {
            exportARAgingToExcel(
              agingData.map((item) => ({
                ...
                <TableBody>
                  {agingData.map((item, index) => (
                    <TableRow key={getRowKey(item, index)}>
                      <TableCell>{getPartyLabel(item, true)}</TableCell>
                      <TableCell>{getDocumentLabel(item, true)}</TableCell>
